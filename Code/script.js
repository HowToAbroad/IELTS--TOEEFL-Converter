...
function read() {
  fetch(
    "https://howtoabroad.github.io/IELTS-TOEFL-Converter/Code/UniqueIBT.json"
  )
    .then((response) => {
  ...
  fetch(
    "https://howtoabroad.github.io/IELTS-TOEFL-Converter/Code/UniqueIELTS.json"
  )
    .then((response) => {
      return response.json();
    })
...
function convertion(IBT) {
  ...
  if (IBT == "") {
    ...
    return {
      PBTvalue: "Enter Valid IBT Value[0-120]",
      CBTvalue: "Enter Valid IBT Value[0-120]",
      IELTSvalue: "Enter Valid IBT Value[0-120]",
      TOEICvalue: "Enter Valid IBT Value[0-120]",
    };
  }
  ...
  return {
    PBTvalue: PBTvalue,
    CBTvalue: CBTvalue,
    IELTSvalue: IELTSvalue,
    TOEICvalue: TOEICvalue,
  };
}
...
function convertion_2(IELTS) {
  ...
  if (IELTS % 1 == 0 || IELTS % 1 == 0.0 || IELTS % 1 == 0.5) {
    if (IELTS == "") {
      ...
      return {
        IBTvalue: "Enter Valid IELTS Score[0-9] with step size of 0.5 ",
        PBTvalue: "Enter Valid IELTS Score[0-9] with step size of 0.5 ",
        CBTvalue: "Enter Valid IELTS Score[0-9] with step size of 0.5 ",
        TOEICvalue: "Enter Valid IELTS Score[0-9] with step size of 0.5 ",
      };
    }
  } else {
    return {
